...
class School:
    ...
    def get_student_by_id(self, student_id):
        return next(
            (student for student in self.students if str(student.student_id) == str(student_id)),
            None
        )

    def get_teacher_by_id(self, employee_id):
        return next(
            (Teacher for Teacher in self.teachers if str(Teacher.employee_id) == str(employee_id)),
            None
        )

    def get_course_by_id(self, course_id):
        return next(
        (course for course in self.courses if str(course.course_id) == str(course_id)),
        None
    )
    ...
    def get_students_with_perfect_attendance(self):
        perfect_attendance = []
        for student in self.students:
            has_perfect_attendance = all(
                all(course_attendance.values())
